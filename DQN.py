...
class train_DQN():
    ...
    def compute_td_loss(self,batch_size, beta):
        ...
        prios = 0.9 * torch.max(td_error)+(1-0.9)*td_error
        loss  = loss.mean()
    ...
    def train(self):
        ...
        for frame_idx in range(self.max_step):
            ...
            if frame_idx+1 % self.target_update_interval == 0:
                self.update_target(self.model, self.target_model)

            if frame_idx+1 % self.save_interval == 0 or frame_idx == self.max_step-1:
